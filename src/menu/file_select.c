...
int menuIndexMax;
int joystickDisable;
u16 musicArray[] = { 0x04, 0x09, 0x0B, 0x21, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2A, 0x2B, 0x2C, 0x2D, 0x2E };
u16 musicIndex;
u8 legacyFortress;
...
void beh_yellow_background_menu_init(void) {
    ...
    gCurrentObject->oMenuButtonScale = 10.0f;
}
...
/**
 * Nintendo 64 logo spin function
 */
void beh_n64_loop(void) {
    gCurrentObject->oFaceAngleYaw += 0x0100;
}
...
//    sMainMenuButtons[MENU_BUTTON_SCORE_FILE_B]->oMenuButtonScale = 0.11111111f;
    ...
    if (save_file_exists(SAVE_FILE_C) == TRUE) {
        sMainMenuButtons[MENU_BUTTON_SCORE_FILE_C] = spawn_object_rel_with_rot(
            scoreButton, MODEL_MAIN_MENU_MARIO_SAVE_BUTTON, bhvMenuButton, -166, 120, -100, 0, -0x8000, 0);
    } else {
        sMainMenuButtons[MENU_BUTTON_SCORE_FILE_C] = spawn_object_rel_with_rot(
            scoreButton, MODEL_MAIN_MENU_MARIO_NEW_BUTTON, bhvMenuButton, -166, 120, -100, 0, -0x8000, 0);
    }
...
void render_copy_menu_buttons(struct Object *copyButton) {
    ...
    if (save_file_exists(SAVE_FILE_A) == TRUE) {
        ...
        sMainMenuButtons[MENU_BUTTON_COPY_FILE_A] = spawn_object_rel_with_rot(
            copyButton, MODEL_MAIN_MENU_MARIO_NEW_BUTTON, bhvMenuButton, 711, 120, -100, 0, -0x8000, 0);
    }
...
*/    // File C
    if (save_file_exists(SAVE_FILE_C) == TRUE) {
        sMainMenuButtons[MENU_BUTTON_COPY_FILE_C] = spawn_object_rel_with_rot(
            copyButton, MODEL_MAIN_MENU_MARIO_SAVE_BUTTON, bhvMenuButton, -166, 120, -100, 0, -0x8000, 0);
    } else {
        sMainMenuButtons[MENU_BUTTON_COPY_FILE_C] = spawn_object_rel_with_rot(
            copyButton, MODEL_MAIN_MENU_MARIO_NEW_BUTTON, bhvMenuButton, -166, 120, -100, 0, -0x8000, 0);
    }
...
*/    // File C
    if (save_file_exists(SAVE_FILE_C) == TRUE) {
        sMainMenuButtons[MENU_BUTTON_ERASE_FILE_C] = spawn_object_rel_with_rot(
            eraseButton, MODEL_MAIN_MENU_MARIO_SAVE_BUTTON, bhvMenuButton, -166, 120, -100, 0, -0x8000, 0);
    } else {
        sMainMenuButtons[MENU_BUTTON_ERASE_FILE_C] = spawn_object_rel_with_rot(
            eraseButton, MODEL_MAIN_MENU_MARIO_NEW_BUTTON, bhvMenuButton, -166, 120, -100, 0, -0x8000, 0);
    }
    ...
    if (save_file_exists(SAVE_FILE_A) == TRUE) {
        sMainMenuButtons[MENU_BUTTON_PLAY_FILE_A] =
            spawn_object_rel_with_rot(gCurrentObject, MODEL_MAIN_MENU_MARIO_SAVE_BUTTON_FADE,
                                      bhvMenuButton, -6400, 1000, 0, 0, 0, 0);
    } else {
        sMainMenuButtons[MENU_BUTTON_PLAY_FILE_A] =
...
*/    // File C
    if (save_file_exists(SAVE_FILE_C) == TRUE) {
        sMainMenuButtons[MENU_BUTTON_PLAY_FILE_C] =
            spawn_object_rel_with_rot(gCurrentObject, MODEL_MAIN_MENU_MARIO_SAVE_BUTTON_FADE,
                                      bhvMenuButton, 1500, 1000, 0, 0, 0, 0);
    } else {
        sMainMenuButtons[MENU_BUTTON_PLAY_FILE_C] = spawn_object_rel_with_rot(
            gCurrentObject, MODEL_MAIN_MENU_MARIO_NEW_BUTTON_FADE, bhvMenuButton, 1500, 1000, 0, 0, 0, 0);
    }
...
void initiate_warp_title(s16 destLevel, s16 destArea, s16 destWarpNode, s32 arg3) {
...
void handle_controller_cursor_input(void) {
    if ((gPlayer1Controller->rawStickY < -60 && menuIndex < menuIndexMax) && joystickDelay == 0) {
        joystickDelay = 1;
        menuIndex++;
        play_sound(SOUND_MENU_CHANGE_SELECT, gGlobalSoundSource);  
    }

    if ((gPlayer1Controller->rawStickY > 60 && menuIndex > 0) && joystickDelay == 0) {
        joystickDelay = 1;
        menuIndex--;
        play_sound(SOUND_MENU_CHANGE_SELECT, gGlobalSoundSource);
    }
    ...
    if (gDialogResponse > 0 && gDialogResponse == 1 && joystickDisable == 1) {
        play_sound(SOUND_MENU_STAR_SOUND, gGlobalSoundSource);
        joystickDisable = 0;
    } else if (gDialogResponse > 0 && gDialogResponse == 2 && joystickDisable == 1) {
        play_sound(SOUND_MENU_STAR_SOUND, gGlobalSoundSource);
        joystickDisable = 0;
    }

    if ((gPlayer1Controller->buttonPressed & A_BUTTON || gPlayer1Controller->buttonPressed & START_BUTTON) && joystickDisable == 0) {
        stop_sound(SOUND_MENU_CHANGE_SELECT, gGlobalSoundSource);
        ...
        if (menuLayer == 0) {
            ...
            switch (menuIndex) {
                case 0:
                    ...
                    break;
                case 1:
                    /*aiModeSeed[0] = random_u16() % 0x30;*/
                    load_main_menu_save_file(sMainMenuButtons[MENU_BUTTON_PLAY_FILE_C], 2);
                    stop_background_music(0x28);
                    play_sound(SOUND_MENU_BOWSER_LAUGH, gGlobalSoundSource);
                    joystickDisable = 1;
                    joystickDelay = 1;
            ...
            switch (menuIndex) {
                case 0:
                    sound_init();
                    play_music(SEQ_PLAYER_LEVEL, musicArray[musicIndex], 0);
                    break;
                case 1:
                    menuIndex = 0;
                    menuLayer = 0;
                    sound_init();
                    play_music(SEQ_PLAYER_LEVEL, 0x28, 0);
                    break;
            }
        } else if (menuLayer == 5) {
            switch (menuIndex) {
...
void print_save_file_star_count(s8 fileIndex, s16 x, s16 y) {
    ...
    if (save_file_exists(fileIndex) == TRUE) {
        ...
        // Print star count
        print_text_fmt_int(x + offset + 16, y, "%0d", starCount);
    } else {
        // Print "new" text
        print_text(x, y, "NEW");
    }
...
void print_main_menu_strings(void) {
    ...
    print_save_file_star_count(SAVE_FILE_A, SAVEFILE_X1, 103 );
//    print_save_file_star_count(SAVE_FILE_B, SAVEFILE_X2, 78);
    print_save_file_star_count(SAVE_FILE_C, SAVEFILE_X2, 103);
//    print_save_file_star_count(SAVE_FILE_D, SAVEFILE_X2, 118);
    ...
    gDPSetEnvColor(gDisplayListHead++, 255, 255, 255, sTextBaseAlpha);
    print_menu_generic_string(MARIOTEXT_X1, 93, textMarioA);
//    print_menu_generic_string(MARIOTEXT_X2, 65, textMarioB);
    print_menu_generic_string(MARIOTEXT_X2, 93, textMarioC);
...
void print_main_lang_strings(void) {
    ...
    gSPDisplayList(gDisplayListHead++, dl_ia_text_begin);
    gDPSetEnvColor(gDisplayListHead++, 255, 255, 255, sTextBaseAlpha);
    centeredX = get_str_x_pos_from_center(103, textScore[sLanguageMode], 10.0f);
...
void print_score_menu_strings(void) {
    ...
    gSPDisplayList(gDisplayListHead++, dl_rgba16_text_begin);
    gDPSetEnvColor(gDisplayListHead++, 255, 255, 255, sTextBaseAlpha);
    print_save_file_star_count(SAVE_FILE_A, 90, 103);
//    print_save_file_star_count(SAVE_FILE_B, 211, 103);
    print_save_file_star_count(SAVE_FILE_C, 211, 103);
//    print_save_file_star_count(SAVE_FILE_D, 211, 119);
    ...
    gSPDisplayList(gDisplayListHead++, dl_menu_ia8_text_begin);
    gDPSetEnvColor(gDisplayListHead++, 255, 255, 255, sTextBaseAlpha);
    print_menu_generic_string(89, 89, textMarioA);
//    print_menu_generic_string(211, 89, textMarioB);
    print_menu_generic_string(211, 89, textMarioC);
...
void jukebox_control(void) {
    if ((menuIndex == 0 && gPlayer1Controller->rawStickX < -40 && musicIndex > 0) && joystickXDelay == 0) {
        joystickXDelay = 1;
        musicIndex--;
        play_sound(SOUND_MENU_CHANGE_SELECT, gGlobalSoundSource);
    }

    if ((menuIndex == 0 && gPlayer1Controller->rawStickX > 40 && musicIndex < 15) && joystickXDelay == 0) {
        joystickXDelay = 1;
        musicIndex++;
        play_sound(SOUND_MENU_CHANGE_SELECT, gGlobalSoundSource);
    }

    if (gPlayer1Controller->rawStickX == 0 && joystickXDelay == 1) {
        joystickXDelay = 0;
    }
}
...
static void print_file_select_strings(void) {
    ...
    if (menuLayer == 0) {
        ...
        switch (menuIndex) {
            case 0:
                print_text_centered(80, 90,   "- START ");
                print_text_centered(80, 72,  "OPTIONS");
                print_text_centered(80, 54,   "MUSIC");
                print_text_centered(80, 36,  "EXTRAS");
                break;
            case 1:
                print_text_centered(80, 90,   "START");
                print_text_centered(80, 72,  "- OPTIONS ");
        ...
        switch (menuIndex) {
            case 0:
                print_text(0, 90, "- FILE1 ");
                print_text(0, 72,  "  FILE2");
                print_text_centered(80, 54,   "ERASE");
                print_text_centered(80, 36,  "GO BACK");
                break;
            case 1:
                print_text(0, 90, "  FILE1");
                print_text(0, 72,  "- FILE2 ");
        ...
        print_save_file_star_count(SAVE_FILE_A, 94, 90);
        print_save_file_star_count(SAVE_FILE_C, 94, 72);
        ...
        switch (menuIndex) {
            case 0:
                print_text(0, 80, "- FILE1 ");
                print_text(0, 63,  "  FILE2");
                print_text_centered(80, 47,  "GO BACK");
                break;
            case 1:
                print_text(0, 80, "  FILE1");
                print_text(0, 63,  "- FILE2 ");
        ...
        print_save_file_star_count(SAVE_FILE_A, 94, 80);
        print_save_file_star_count(SAVE_FILE_C, 94, 63);
        ...
        switch (menuIndex) {
           case 0:
                print_text_centered(80, 80, "- ABOUT ");
                print_text_centered(80, 47, "GO BACK");
                if (sSoundMode == 0) {
                    print_text_centered(80, 63,  "STEREO");
                } else if (sSoundMode == 3) {
                    print_text_centered(80, 63,  "MONO");
                }
                break;
            case 1:
                print_text_centered(80, 80, "ABOUT");
        ...
        switch (menuIndex) {
            case 0:
                print_text_fmt_int(44, 63, "- %02d  ", musicArray[musicIndex] - 34);
                print_text_centered(80, 47, "GO BACK");
                break;
            case 1:
                print_text_fmt_int(44, 63, " %02d", musicArray[musicIndex] - 34);
        ...
        print_text_centered(80, 80, "MUSIC");
        jukebox_control();
    } else if (menuLayer == 5) {
        menuIndexMax = 1;
        ...
        switch (menuIndex) {
            case 0:
                print_text_centered(80, 72, "- SPECIAL ");
                print_text_centered(80, 54, "GO BACK");
                break;
            case 1:
                print_text_centered(80, 72, "SPECIAL");
                print_text_centered(80, 54, "- GO BACK ");
        ...
        switch (menuIndex) {
            case 0:
                print_text(0, 72, "- FILE1 ");
                print_text(0, 54,  "  FILE2");
                break;
            case 1:
                print_text(0, 72, "  FILE1");
                print_text(0, 54,  "- FILE2 ");
        ...
        print_save_file_star_count(SAVE_FILE_A, 94, 72);
        print_save_file_star_count(SAVE_FILE_C, 94, 54);
    }
}
...
s32 lvl_decider(UNUSED s32 arg, UNUSED s32 unused) {
    if (legacyFortress == 0) {
        return 1;
    } else {
        return -1;
